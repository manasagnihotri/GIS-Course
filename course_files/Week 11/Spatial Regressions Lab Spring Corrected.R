...
##Merge to your census tract data (data not here but general data with a GEOID connector)
...
spatial_regression_data <- spatial_regression_data %>%
  rename(
    ...
    marijuana_arrest_rate = mrjn__2               # your dependent variable
  )

spatial_regression_data_clean <- spatial_regression_data %>%
    filter(
      !is.na(marijuana_arrest_rate),
...
neighbors <- poly2nb(spatial_regression_data_clean, queen = TRUE, snap = 1e-6)
weights <- nb2listw(neighbors, style = "W", zero.policy = TRUE)

##Make a Log of Marijuana Arrest Rate given the data might be skewed
spatial_regression_data_clean$marijuana_arrest_rate_log <- log(spatial_regression_data_clean$marijuana_arrest_rate + 1)

##Spatial Lag Model
lag_model <- lagsarlm(
  marijuana_arrest_rate ~ prop_within_50ft + percent_black + percent_hispanic +
...
# Compare AIC
AIC(linear_regression_1, lag_model, error_model, durbin_model)

##Look at Direct and Indirect Effects of the Durbin Model
