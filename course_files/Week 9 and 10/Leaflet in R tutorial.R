...
#can find points using https://www.maps.ie/coordinates.html
...
states <- readOGR(dsn=tmp,layer = "cb_2016_us_state_20m", GDAL1_integer64_policy = TRUE)
...
#show polygon shapefile in leaflet map
...
#chorepleth function for colors and legend
#colorQuantile is a function that defines how another function will 
#create quantile breaks and what color scheme to assign.

qpal <- colorQuantile("RdYlBu", neStates@data$ALAND, n = 5,reverse=TRUE)
#arg1 is brewerpal color scheme, arg2 is possible values that can be mapped, 
#n arg is number of bins, reverse is used to reverse color scheme.

qpal(neStates@data$ALAND) #returns vector of colors that define numeric data

#Note: for non numeric color categories use color <- colorFactor(topo.colors(7), data$column)
...
#This is an example of what you will learn in a QMSS data viz course
#We will also cover an introduction to shiny apps towards the end of the semester.
