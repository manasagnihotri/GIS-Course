...
####If simplifed mapping where the neighbor issue is not a problem
...
###Or change to Rook Contiguity
##neighbors_rook <- poly2nb(nyc_tracts_sf, queen = FALSE)
##weights_rook <- nb2listw(neighbors_rook, style = "W", zero.policy = TRUE)
...
lisa_map <- tm_shape(nyc_tracts_sp_clean) +
  tm_polygons(
    fill = "lisa_cluster",
...
tmap_save(lisa_map, filename = "lisa_map_arrests.png", width = 8, height = 6, units = "in", dpi = 300)
